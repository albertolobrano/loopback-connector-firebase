  ...
  if (filterKeys && filterKeys.length > 0) {
    for (var idx = 0; idx < filterKeys.length; idx++) {
      ...
      if (filterKeys[idx] === 'where') {
        ...
        var whereObj = filter.where;
        for (var key in whereObj){
          if (typeof whereObj[key] !== 'object') {
            newRef = newRef.orderByChild(key).startAt(whereObj[key]).endAt(whereObj[key]);
          } else {
            for (var subkey in whereObj[key]){
              if (subkey === 'gt' || subkey === 'gte') {
                newRef = newRef.orderByChild(key).startAt(whereObj[key][subkey]);
              }
              if (subkey === 'lt' || subkey === 'lte') {
                newRef = newRef.orderByChild(key).endAt(whereObj[key][subkey]);
              }
            }
          }
        }
      ...
      if (filterKeys[idx] === 'limit') {
        ...
        //newRef= newRef.limitToFirst(filter.limit);
      }
